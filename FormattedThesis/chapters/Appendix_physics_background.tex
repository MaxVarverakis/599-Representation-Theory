...
For a linear operator $\hat{A}$ on $\mathbf{H}$, the action of $\hat{A}$ on a ket $\ket{\psi}$ is denoted by $\ket{\hat{A}\psi} = \hat{A}\ket{\psi}$. Moreover, for a bra $\bra{\phi}$, we have a corresponding linear functional $\bra{\phi}\hat{A}$ defined by
\begin{align}
...
\end{align}
% which can be rewritten in bra-ket notation as
% \begin{align}
%     \braket{A} = \bra{\Psi}\hat{A}\ket{\Psi}.
% \end{align}
% The fact that $\hat{A}$ is Hermitian guarantees that the expectation value is real.
...
\section{Conserved quantities in quantum mechanics}\label{sec:conserved_quantities}
    ...
    % An alternative description of commuting operators is presented as a theorem.
    % \begin{theorem}\label{thm:commuting_ops}
    % Let $\hat{A}$ and $\hat{B}$ be commuting linear operators \textcolor{red}{such that $\hat{A}$ has a nondegenerate set of eigenvectors}. Then $\hat{A}$ and $\hat{B}$ have a mutual set of eigenvectors.
    % \end{theorem}
    % \begin{proof}
    %     To prove this, consider some $\ket{a}$ such that $\hat{A}\ket{a} = a\ket{a}$. By assumption, we have
    %     \begin{align}
    %         \hat{A}\hat{B}\ket{a} = \hat{B}\hat{A}\ket{a} = a\hat{B}\ket{a} \implies 
    %     \end{align}
    % \end{proof}