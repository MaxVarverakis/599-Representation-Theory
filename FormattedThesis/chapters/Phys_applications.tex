...
\begin{align}
    \sum_{n\in\Z} U^n(\phi){(U^n(\phi'))}^\dagger = \sum_{n=-\infty}^\infty e^{-in(\phi-\phi')} = 2\sum_{n=0}^{\infty}\cos(n(\phi-\phi')) = \delta(\phi-\phi'),\label{eq:completeness}
\end{align}
where the last equality comes from the fact that the Fourier series of the Dirac delta is a sum over all frequencies of cosines (since it is an even distribution). The factor of $2\pi$ is omitted since it will vanish when integrating over $\phi$ with the appropriate normalization. These relations will be exploited in \cref{sub:SO2_decomp}.
% \begin{align*}
...
\end{align*}
Thus, the state $\ket{\phi}$ can be decomposed into a linear combination of the eigenvectors of $J$:
\begin{align*}
