...
\end{align}
where the $p_i - A_i(\vec{r}_i)$ term is known as the \textit{canonical momentum} of the system. This is a required modification since we must account for the motion of the anyons in the presence of the gauge potential in addition to their mechanical momentum.
...
\begin{align*}
    ...
    &= -2\alpha\sum_{j\neq i}\frac{(\vec{p}_i\times\vec{r}_{ij})\cdot\hat{z}}{r_{ij}^2}.
\end{align*}

For each $j$, there is a corresponding term in \cref{eq:full_H} with
\begin{align*}
    -2\alpha \frac{(\vec{p}_j\times\vec{r}_{ji})\cdot\hat{z}}{r_{ji}^2} = 2\alpha \frac{(\vec{p}_j\times\vec{r}_{ij})\cdot\hat{z}}{r_{ij}^2}.
\end{align*}
Then, summing over all $i$ and $j$ yields the second-to-last term in \cref{eq:full_H} since each of these terms can be combined to give:
\begin{align*}
    -2\alpha\sum_{j\neq i}\frac{(\vec{p}_i\times\vec{r}_{ij} - \vec{p}_j\times\vec{r}_{ij})\cdot\hat{z}}{r_{ij}^2} = -2\alpha\sum_{j\neq i}\frac{(\vec{p}_{ij}\times\vec{r}_{ij})\cdot\hat{z}}{r_{ij}^2} = -2\alpha\sum_{j\neq i}\frac{\vec{\ell}_{ij}}{r_{ij}^2}.
\end{align*}