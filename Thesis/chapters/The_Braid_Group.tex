...
As explained in~\cite{Delaney2016}, we can use $U$ from the construction of the (unreduced) Burau representation to show that there are no choices of the parameter $t$ such that the representation matrices are unitary. However, we can use the Burau representation matrices to construct a unitary representation of $B_n$~\cite{Squier1984}.

% However, we can use the reduced Burau representation matrices to construct a unitary representation of $B_n$.

First, let $t=s^2$ for some $s\in\C^*$. In other words, we are restricting $t$ to be a square of a nonzero complex number. Define $n\times n$ matrix
\begin{align}
    ...
    % \end{bmatrix},
\end{align}
where diag denotes the diagonal matrix with the given diagonal entries. Furthermore, define $\psi_n^\mathbf{s}(\beta) = P_{n-1}\psi_n(\beta)\iv{\left( P_{n-1} \right)}$ for $\beta\in B_n$. Note that
\begin{align*}
    \iv{\left( P_{n-1} \right)} = \textrm{diag}(1,s^{-1},s^{-2},\dots,s^{-(n-1)}).
\end{align*}
...
\end{align*}
for all $j=1,2,\dots,n-1$. Therefore, $\textrm{Re}(s)>0$. Furthermore, we place a restriction on $X$ by observing that
\begin{align*}
    {\left( X^\dagger X \right)}_{i,j} &= \sum_{k=1}^{n-1} \overline{x}_{k,i}x_{k,j} = -\delta_{i,j\pm 1},
\end{align*}
where $\delta_{ij}$ is the Kronecker delta.

For such a choice of $s$ (hence $X$ and $t$) and any braid $\beta\in B_n$, the corresponding matrix $X\psi_\textbf{s}(\beta)\iv{X}$ is unitary.

% \begin{align*}
%     % X\psi_\textbf{s}(\beta)\iv{X} &= XP_{n-1}\psi_n^\mathbf{r}(\beta)\iv{\left( P_{n-1} \right)}\iv{X}
%     {\left( \psi_\textbf{s}(\beta) \right)}^\dagger J_{n-1}\psi_\textbf{s}(\beta) = J_{n-1} \\
%     X\psi_\textbf{s}(\beta)\iv{X} \\
% \end{align*}
